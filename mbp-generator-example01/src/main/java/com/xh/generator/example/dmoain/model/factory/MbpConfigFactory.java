package com.xh.generator.example.dmoain.model.factory;
...
import com.xh.generator.example.dmoain.model.GlobalConfig;
import com.xh.generator.example.dmoain.model.PackageConfig;
import com.xh.generator.example.dmoain.model.StrategyConfig;
import com.xh.generator.example.dmoain.model.TemplateConfig;
...
public class MbpConfigFactory {
    ...
    public static MbpConfigFactory getSingleton() {
        return MbpConfigFactory.Inner.instance;
    }
     ...
     * @param globalConfig
     */
    public void getGlobalConfig(com.baomidou.mybatisplus.generator.config.GlobalConfig.Builder builder, GlobalConfig globalConfig) {
        // 设置作者
        builder.author(globalConfig.getAuthor())
                // 日期类型的字段使用哪个类型，默认是 java8的 日期类型，此处改为 java.util.date
                .dateType(DateType.ONLY_DATE)
                // 指定输出目录
                .outputDir(globalConfig.getSourcePath());
     ...
     * @param packageConfig
     */
    public void getPackageConfig(com.baomidou.mybatisplus.generator.config.PackageConfig.Builder builder, PackageConfig packageConfig) {
        // 设置父包名
        builder.parent(packageConfig.getPackagePath())
                //Mapper XML 包名
                .xml("mapper")
                // 设置mapperXml生成路径
                .pathInfo(Collections.singletonMap(OutputFile.xml, packageConfig.getResourcePath()));
    }
     ...
     */
    public void getTemplateConfig(com.baomidou.mybatisplus.generator.config.TemplateConfig.Builder builder, TemplateConfig templateConfig) {
        ...
        fileSuffixDict.entrySet().stream().filter(entry -> !templateConfig.getSelectedOutputFiles().contains(entry.getValue())).forEach(entry -> builder.disable(entry.getKey()));
    }
     ...
     */
    public void getStrategyConfig(com.baomidou.mybatisplus.generator.config.StrategyConfig.Builder builder, StrategyConfig strategyConfig) {
        builder.addInclude(strategyConfig.getTableNames()); // 设置需要生成的表名
        // 设置过滤表前缀
        if (strategyConfig.getTablePrefixs() != null && !strategyConfig.getTablePrefixs().isEmpty()) {
            builder.addTablePrefix(strategyConfig.getTablePrefixs());
        }

        this.entityConfig(builder.entityBuilder(), strategyConfig);
    ...
    private void entityConfig(Entity.Builder entityBuilder, StrategyConfig strategyConfig) {
        // 实体类策略配置
        ...
        if (strategyConfig.isEnableLombok()) {
            entityBuilder.enableLombok();
        }

        // 开启生成实体时生成字段注解。会在实体类的属性前，添加[@TableField("nickname")]
        if (strategyConfig.isEnableTableFieldAnnotation()) {
    ...
    private void serviceConfig(com.baomidou.mybatisplus.generator.config.builder.Service.Builder serviceBuilder) {
